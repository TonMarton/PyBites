...
class Promo:
    ...
    def __init__(self):
        ...
        self.all_bites: dict[int, str] = BITES.copy()
        self.bites_done: set[int] = BITES_DONE.copy()

    def _pick_random_bite(self) -> int:
        available_keys = list(self.all_bites.keys() - self.bites_done)
        if not available_keys:
            raise NoBitesAvailable(Exception())
        return random.choice(available_keys)

    def new_bite(self):
        """Get  a random Bite using _pick_random_bite,
           add it to self.bites_done, then return it"""
        bite_key = self._pick_random_bite()
        self.bites_done.add(bite_key)
        return bite_key
